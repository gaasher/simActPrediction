...
class APDataloader(pl.LightningDataModule):
    def __init__(self, path, batch_size, shuffle, num_workers=0, do_transform=False):
        ...
        self.train_dataset = NIHDataset(path, 'train', do_transform)
        self.val_dataset = NIHDataset(path, 'val', do_transform=False)
        self.test_dataset = NIHDataset(path, 'test', do_transform = False) # hard code this for test set
